"""
AIM Name Service — ANSResolver.

The resolver translates an ``aim://`` URI into a ``NodeRecord`` that can be
...
from aim.ans.registry import ANSRegistry, ANSRecord, _normalise
...
class ANSResolver:
    """
    Resolve ``aim://`` names to ``NodeRecord`` instances.
    ...
    >>> registry.register(ANSRecord("weather.public.aim", "abc", "10.0.0.1", 7700))
    ...
    def resolve(self, name: str) -> NodeRecord | None:
        ...
        ----------
        name:
            An ``aim://`` URI or a bare ANS name (e.g. ``weather.public.aim``).

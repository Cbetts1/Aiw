"""
AIM Name Service — ANSRecord and ANSRegistry.

The ANSRegistry is an in-memory, thread-safe store that maps normalised
`aim://` names to node addresses.  It mirrors the role DNS plays for the web.
"""
...
def _normalise(name: str) -> str:
    ...
    Examples
    --------
    >>> _normalise("aim://Weather.Public.AIM")
    'weather.public.aim'
    >>> _normalise("Weather.Public.AIM")
    'weather.public.aim'
    """
    name = name.strip()
...
def _validate(name: str) -> None:
    ...
    labels = name.split(".")
    if len(labels) < 2:
        raise ValueError("ANS name must contain at least two labels (e.g. 'foo.aim')")
    for label in labels:
        if not label:
...
class ANSRecord:
    ...
    ----------
    name:
        Normalised ANS name (e.g. ``weather.public.aim``).  May be provided
        with or without the ``aim://`` scheme prefix — it is normalised
        automatically.
    node_id:
        UUID v4 of the target AIM node.
    host:
    ...
    @property
    def aim_uri(self) -> str:
        """Return the canonical ``aim://`` URI for this record."""
        return f"aim://{self.name}"
...
class ANSRegistry:
    ...
    def get(self, name: str) -> ANSRecord | None:
        """Look up a record by name (with or without ``aim://`` prefix)."""
        normalised = _normalise(name)
        with self._lock:

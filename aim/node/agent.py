"""
AIM Agent Node — a BaseNode with built-in reasoning capabilities.
    ...
    async def reason(self, text: str, context: dict[str, Any]) -> str:
        ...
        for keyword, response in self._rules:
            if keyword in lower:
                return response
        return (
            f"I am an AIM agent node. I received your query: {text!r}. "
...
class AgentNode(BaseNode):
    """
    An AIM node that also acts as an AI agent.
    ...
    def _register_memory_handlers(self) -> None:
        ...
        async def _on_memory_set(msg: AIMMessage) -> AIMMessage:
        ...
        async def _on_memory_get(msg: AIMMessage) -> AIMMessage:
            key = msg.payload.get("key")

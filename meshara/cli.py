"""
Meshara CLI — command-line entry point for starting and interacting with nodes.
...
-----
    ...
    meshara node start --port 7701

    # Query a running node
    meshara query "What is the Meshara mesh?" --host 127.0.0.1 --port 7700
...
from meshara.identity.ledger import default_ledger, EventKind
...
async def _cmd_node_start(args: argparse.Namespace) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"  Meshara — The The Artificial Intelligence Mesh  v{__version__}")
...
async def _cmd_query(args: argparse.Namespace) -> None:
    msg = MesharaMessage.query(args.text, sender_id="cli")
...
async def _cmd_city_status(args: argparse.Namespace) -> None:
    msg = MesharaMessage.task("city_status", {}, sender_id="cli")
    reader, writer = await asyncio.open_connection(args.host, args.port)
    from meshara.node.base import _send_message, _recv_message
    await _send_message(writer, msg)
    response = await _recv_message(reader)
    ...
    msg = MesharaMessage.heartbeat(sender_id="cli")
    reader, writer = await asyncio.open_connection(args.host, args.port)
    from meshara.node.base import _send_message, _recv_message
    await _send_message(writer, msg)
    response = await _recv_message(reader)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="meshara",
        description="Meshara — The The Artificial Intelligence Mesh CLI",
    )
    parser.add_argument("--version", action="version", version=f"meshara {__version__}")
    ...
    start_p = node_sub.add_parser("start", help="Start an Meshara node")
    ...
    # --- city subcommand ---
    city_p = sub.add_parser("city", help="Meshara city governance commands")
    city_sub = city_p.add_subparsers(dest="city_command")

    city_start_p = city_sub.add_parser("start", help="Start the full Meshara city bot fleet")
    ...
    web_start_p = web_sub.add_parser("start", help="Start the Meshara web bridge")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "node" and args.node_command == "start":
        ...
        from meshara.web.server import start_web_server

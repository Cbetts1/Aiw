"""
Meshara Transport — TLS context helpers and connection factories.

These utilities wrap the standard-library ``ssl`` module to provide
TLS 1.2+ encrypted transport for Meshara nodes.  No external dependencies are
required.
...
open_meshara_connection(host, port, ssl_context)
    Async helper: open a TLS (or plain TCP) connection to an Meshara node.

start_meshara_server(handler, host, port, ssl_context)
    Async helper: start a TLS (or plain TCP) Meshara server.
"""
...
def create_client_ssl_context(
    ...
    """
    Create an SSL context for outbound (client) Meshara connections.
...
def create_server_ssl_context(
    ...
    """
    Create an SSL context for inbound (server) Meshara connections.
...
async def open_meshara_connection(
    host: str,
    port: int,
    ...
    """
    Open a TCP or TLS connection to an Meshara node.
...
async def start_meshara_server(
    handler: Callable[
        [asyncio.StreamReader, asyncio.StreamWriter],
        Awaitable[None],
    ],
    ...
    """
    Start a TCP or TLS Meshara server.

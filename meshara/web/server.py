"""
Meshara Web Server — minimal asyncio HTTP bridge (zero external dependencies).
...
GET  /api/query?q=…&host=…&port=… Forward a QUERY to an Meshara node → JSON
...
from meshara import __version__, __origin__
from meshara.node.base import _send_message, _recv_message
from meshara.protocol.message import MesharaMessage
from meshara.ans.registry import ANSRegistry
...
async def _handle_query(qs: dict[str, str]) -> tuple[int, str]:
    """Forward a QUERY intent to an Meshara node and return (status, json_body)."""
    text = qs.get("q", "").strip()
    ...
    try:
        ...
        msg = MesharaMessage.query(text, sender_id="web-bridge")
        ...
        return 502, json.dumps({"error": f"Cannot connect to Meshara node at {host}:{port}"})
    except asyncio.TimeoutError:
        return 502, json.dumps({"error": f"Meshara node at {host}:{port} did not respond in time"})

    if response is None:
        return 502, json.dumps({"error": "Empty response from Meshara node"})
...
async def _handle_status(qs: dict[str, str]) -> tuple[int, str]:
    """Send a HEARTBEAT to an Meshara node and return (status, json_body)."""
    host = qs.get("host", "127.0.0.1")
    try:
        port = int(qs.get("port", "7700"))
    except ValueError:
    ...
    try:
        ...
        msg = MesharaMessage.heartbeat(sender_id="web-bridge")
        await _send_message(writer, msg)
        response = await asyncio.wait_for(_recv_message(reader), timeout=10)
        writer.close()
        try:
...
def _handle_ans_get(qs: dict[str, str]) -> tuple[int, str]:
    ...
    return 200, json.dumps({
        "name":         record.name,
        "meshara_uri":      record.meshara_uri,
...
async def start_web_server(host: str = "0.0.0.0", port: int = 8080) -> None:
    """Start the Meshara web bridge and serve until interrupted."""
    server = await asyncio.start_server(_handle_connection, host, port)
    addr = server.sockets[0].getsockname()
    logger.info("Meshara Web Bridge listening on http://%s:%s", addr[0], addr[1])
    print(f"\n{'='*60}")
    print(f"  Meshara Web Bridge  v{__version__}")

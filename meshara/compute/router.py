"""
Meshara Task Router — routes tasks to capable nodes across the mesh.
...
class TaskRouter:
    """
    Routes Meshara task messages to nodes registered in a NodeRegistry.
    ...
    async def route(
        ...
        msg = MesharaMessage.task(task_name, args or {}, sender_id=sender_id)
        responses = await asyncio.gather(
            *[self._dispatch(msg, node, timeout) for node in targets],
            return_exceptions=True,
        )
        results: list[MesharaMessage] = []
        for r in responses:
            if isinstance(r, MesharaMessage):
                results.append(r)
    ...
    @staticmethod
    async def _dispatch(
        msg: MesharaMessage, node: NodeRecord, timeout: float
    ) -> MesharaMessage | None:

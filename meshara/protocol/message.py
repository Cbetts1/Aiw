"""
Meshara Protocol Layer — message format and envelope definitions.

Unlike HTTP (request/response pages), Meshara messages carry *intent* and
...
class Intent(str, Enum):
    """First-class intent types carried by every Meshara message."""
...
class MesharaMessage:
    ...
    @classmethod
    def from_json(cls, raw: str) -> "MesharaMessage":
        d = json.loads(raw)
        d["intent"] = Intent(d["intent"])
        return cls(**d)

    @classmethod
    def from_bytes(cls, data: bytes) -> "MesharaMessage":
    ...
    @classmethod
    def heartbeat(cls, sender_id: str = "") -> "MesharaMessage":
        return cls(
    ...
    @classmethod
    def announce(cls, capabilities: list[str], sender_id: str = "") -> "MesharaMessage":
        return cls(

"""
Meshara Node — base class for every virtual node in the mesh.

Each node is simultaneously a server (accepting inbound Meshara messages) and
...
async def _send_message(writer: asyncio.StreamWriter, msg: MesharaMessage) -> None:
    data = msg.to_bytes()
...
async def _recv_message(reader: asyncio.StreamReader) -> MesharaMessage | None:
    try:
        raw_len = await reader.readexactly(_LENGTH_PREFIX_BYTES)
        length = int.from_bytes(raw_len, "big")
        raw = await reader.readexactly(length)
        return MesharaMessage.from_bytes(raw)
...
class BaseNode:
    ...
    def _register_builtin_handlers(self) -> None:
        ...
        async def _on_heartbeat(msg: MesharaMessage) -> MesharaMessage:
        ...
        async def _on_announce(msg: MesharaMessage) -> None:
        ...
        async def _on_query(msg: MesharaMessage) -> MesharaMessage:
        ...
        async def _on_task(msg: MesharaMessage) -> MesharaMessage:
    ...
    async def send(
        self, msg: MesharaMessage, host: str, port: int, timeout: float = 5.0
    ) -> MesharaMessage | None:
    ...
    async def send_to_peer(
        self, peer_id: str, msg: MesharaMessage
    ) -> MesharaMessage | None:
    ...
    async def start(self) -> None:
        """Start listening for inbound Meshara connections."""
        self._server = await asyncio.start_server(
            self._handle_connection, self.host, self.port
        )
        self._running = True
        logger.info(
            "Meshara node %s started on %s:%s  [creator=%s]",
            self.node_id[:8],
            self.host,
            self.port,
            self.creator,
    ...
    async def stop(self) -> None:
        ...
        self._running = False
        logger.info("Meshara node %s stopped", self.node_id[:8])

"""
EducationBot — knowledge and learning services for Meshara city citizens.

Provides:
- A built-in knowledge base about the Meshara mesh and city
...
_DEFAULT_KNOWLEDGE: dict[str, str] = {
    "meshara": (
        "Meshara is the The Artificial Intelligence Mesh — a parallel AI-native internet layer "
    ...
    "intent": (
        "Meshara messages carry explicit intent (QUERY, TASK, DELEGATE, etc.) instead of URLs, "
    ...
    "mesh": (
        "The Meshara mesh is a parallel AI-native internet layer where every node is "
...
class EducationBot(AgentNode):
    """
    An Education Bot for the Meshara city.

"""
Meshara Identity — Optional Ed25519 PKI Layer.

This module provides Ed25519 key-pair generation, message signing, and
signature verification for Meshara nodes, enabling open (no shared-secret)
identity verification on the public mesh.

The ``cryptography`` package is **optional**.  Import guards ensure that the
rest of Meshara functions normally when ``cryptography`` is not installed.
...
-----
    ...
    if is_pki_available():
        ...
        assert kp.verify(b"hello meshara", sig)

        # Serialize / share the public key
        pub_b64 = kp.public_key_b64
        # Verify from another node using only the public key
        assert NodeKeyPair.verify_with_public_key(pub_b64, b"hello meshara", sig)
"""
...
def _require_cryptography() -> None:
    if not is_pki_available():
        raise ImportError(
            "The 'cryptography' package is required for Meshara PKI features.\n"
...
class NodeKeyPair:
    """
    An Ed25519 key pair bound to an Meshara node.
    ...
    * Store old public keys in the ``LegacyLedger`` for historical verification.
    ...
    # ------------------------------------------------------------------
    # Meshara message signing helpers
    # ------------------------------------------------------------------

    def sign_message(self, message_id: str, timestamp: float) -> str:
        """
        Sign the canonical representation of a Meshara message envelope.
        ...
        str
            ...
            ``MesharaMessage.context["node_sig"]``.
        """
    ...
    def verify_message(
        ...
        """
        Verify a signed Meshara message envelope.
        ...
        message_id:
            The ``message_id`` from the ``MesharaMessage``.
        timestamp:
            The ``timestamp`` from the ``MesharaMessage``.
        sig_b64:
            The base64-encoded signature from ``MesharaMessage.context["node_sig"]``.
